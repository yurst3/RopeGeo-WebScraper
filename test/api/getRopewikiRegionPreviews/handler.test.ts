...
import { RopewikiRegionPreviewsResult } from 'ropegeo-common/classes';
import { handler } from '../../../src/api/getRopewikiRegionPreviews/handler';
...
let mockGetRopewikiRegionPreviews: jest.MockedFunction<typeof import('../../../src/api/getRopewikiRegionPreviews/util/getRopewikiRegionPreviews').default>;
...
jest.mock('../../../src/api/getRopewikiRegionPreviews/util/getRopewikiRegionPreviews', () => ({
    __esModule: true,
    default: jest.fn(),
}));
...
describe('getRopewikiRegionPreviews handler', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        ...
        mockGetRopewikiRegionPreviews = require('../../../src/api/getRopewikiRegionPreviews/util/getRopewikiRegionPreviews').default;
        ...
        mockGetRopewikiRegionPreviews.mockResolvedValue(new RopewikiRegionPreviewsResult([], null));
    });
    ...
    it('returns 200 and RopewikiRegionPreviewsResult with CORS headers', async () => {
        const id = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890';
        const mockResult = new RopewikiRegionPreviewsResult([], null);
        mockGetRopewikiRegionPreviews.mockResolvedValue(mockResult);
        ...
        expect(mockGetRopewikiRegionPreviews).toHaveBeenCalled();
    ...
    it('returns 404 when no region exists with the given id', async () => {
        ...
        expect(mockGetRopewikiRegionPreviews).not.toHaveBeenCalled();
    ...
    it('handles getDatabaseConnection failure and returns 500', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error in getRopewikiRegionPreviews handler:', error);
        expect(mockGetRopewikiRegionPreviews).not.toHaveBeenCalled();
    ...
    it('handles getRopewikiRegionPreviews failure and returns 500', async () => {
        const error = new Error('Query failed');
        mockGetRopewikiRegionPreviews.mockRejectedValue(error);
