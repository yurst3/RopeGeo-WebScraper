...
describe('saveImageData', () => {
    ...
    describe('lambdaSaveImageData', () => {
        it('throws when IMAGE_BUCKET_NAME is not set', async () => {
            ...
            await expect(
                lambdaSaveImageData(imageDataId, sourceUrl, previewBuffer, bannerBuffer, fullBuffer, logger),
        ...
        it('when not local, uploads all three and returns ImageData with URLs', async () => {
            ...
            expect(mockUploadImageDataToS3).toHaveBeenCalledTimes(3);
        ...
        it('when upload fails, returns ImageData with errorMessage and logs', async () => {
            delete process.env.DEV_ENVIRONMENT;
            mockUploadImageDataToS3.mockImplementation((key, _body, _ct, errors?: string[]) => {
