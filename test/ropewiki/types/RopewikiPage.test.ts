...
describe('RopewikiPage', () => {
    ...
    describe('toDbRow', () => {
        it('converts RopewikiPage to database row format', () => {
            ...
            expect(dbRow.overallLength).toBe(1);
            expect(dbRow.approachLength).toBeNull();
            expect(dbRow.approachElevGain).toBeNull();
            expect(dbRow.descentLength).toBeNull();
            expect(dbRow.descentElevGain).toBeNull();
        ...
        it('converts undefined fields to null in database row', () => {
            ...
            expect(dbRow.overallLength).toBeNull();
            expect(dbRow.approachLength).toBeNull();
            expect(dbRow.approachElevGain).toBeNull();
            expect(dbRow.descentLength).toBeNull();
            expect(dbRow.descentElevGain).toBeNull();
